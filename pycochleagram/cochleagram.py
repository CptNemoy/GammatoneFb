...
from pycochleagram import utils as utils
...
def cochleagram(signal, sr, n, low_lim, hi_lim, sample_factor,
  ...
  #print(f' calling log on freqs. {freqs}')
  ...
  for i in range(batch_signal.shape[0]):
    ...
    if ret_mode == 'envs' or ret_mode == 'all':
      ...
      temp_sb = sb.generate_analytic_subbands(temp_signal_flat, filts, padding_size=padding_size,
          fft_mode=fft_mode)
      env_sb = apply_envelope_nonlinearity(temp_sb, nonlinearity)
    ...
    if i == 0:
        if( np.isrealobj(temp_sb) == False):
          sb_out = np.zeros(([batch_signal.shape[0]] + list(temp_sb.shape)), dtype=complex)
        else:
          sb_out = np.zeros(([batch_signal.shape[0]] + list(temp_sb.shape)), dtype=float)
        env_sb_out = np.zeros(([batch_signal.shape[0]] + list(temp_sb.shape)), dtype=float)

    sb_out[i] = temp_sb
    
    if ret_mode == 'analytic':
       env_sb_out[i] = env_sb
